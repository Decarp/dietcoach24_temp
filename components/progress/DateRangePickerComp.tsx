...
const getUnixTimestamp = (date: Dayjs | null): string => (date ? date.unix().toString() : 'N/A');
...
export default function DateRangePickerComp({
    ...
    const { setSelectedBasketIds, setSelectedComparisonBasketIds } = useCounterStore((state) => state);
    ...
    const [range1Dates, setRange1Dates] = useState<[Dayjs | null, Dayjs | null]>([null, null]);
    const [range2Dates, setRange2Dates] = useState<[Dayjs | null, Dayjs | null]>([null, null]);
    ...
    useEffect(() => {
        if (range2Baskets) {
            setSelectedComparisonBasketIds(range2Baskets.map((basket) => basket.basketId));
        }
        ...
        <LocalizationProvider dateAdapter={AdapterDayjs} adapterLocale="de">
            <div className="grid grid-cols-2 gap-28 mt-6">
                <section className="space-y-2">
                    ...
                    <DateRangePicker value={range1Dates} onChange={(newValue) => setRange1Dates(newValue)} />
                    <p className="text-xs text-gray-500">
                        {range1Weeks} Wochen ausgewählt, darin {range1BasketCount} Einkäufe mit {range1ProductCount}{' '}
                        Lebensmitteln
                    </p>
                </section>
                ...
                <section className="space-y-2">
                    ...
                    <DateRangePicker value={range2Dates} onChange={(newValue) => setRange2Dates(newValue)} />
                    <p className="text-xs text-gray-500">
                        {range2Weeks} Wochen ausgewählt, darin {range2BasketCount} Einkäufe, {range2ProductCount}{' '}
                        Lebensmitteln
                    </p>
                </section>
            </div>
