...
const ChartEnergyMacro = dynamic(() => import('@/components/charts/ChartEnergyMacro'), {
    ssr: false,
});
const ChartEnergyCategories = dynamic(() => import('@/components/charts/ChartEnergyCategories'), {
    ssr: false,
});
const ChartEnergyMacroCategories = dynamic(() => import('@/components/charts/ChartEnergyMacroCategories'), {
    ssr: false,
});
const ChartEnergyMicroCategories = dynamic(() => import('@/components/charts/ChartEnergyMicroCategories'), {
    ssr: false,
});
...
const Analysis = () => {
    ...
    const { isLoading: isLoadingBasketProductsComparisonNew, data: basketProductsComparisonNew } = useQuery<
        BasketProduct[]
    >({
        queryKey: ['basketProductsComparisonNew', selectedBasketIds],
        queryFn: () => fetchBasketProducts(patientId, selectedBasketIds, session?.accessToken || ''),
        enabled: selectedBasketIds.length > 0 && !!session?.accessToken,
    });

    const { isLoading: isLoadingBasketProductsComparisonOld, data: basketProductsComparisonOld } = useQuery<
        BasketProduct[]
    >({
        queryKey: ['basketProductsComparisonOld', selectedComparisonBasketIds],
        queryFn: () => fetchBasketProducts(patientId, selectedComparisonBasketIds, session?.accessToken || ''),
        enabled: selectedComparisonBasketIds.length > 0 && !!session?.accessToken,
    });

    const [isProcessing, setIsProcessing] = useState(true);

    const [percentageDifferencePrimary, setPercentageDifferencePrimary] = useState<number | null>(null);

    const [percentageDifferenceSecondary, setPercentageDifferenceSecondary] = useState<number | null>(null);
    ...
    const chartEnergyCategoriesData = useMemo(() => {
        const data = getChartEnergyCategoriesData(basketProductsComparisonNew || []);
        return data.sort((a, b) => a.name.localeCompare(b.name));
    }, [basketProductsComparisonNew]);

    const chartComparisonEnergyCategoriesData = useMemo(() => {
        const data = getChartEnergyCategoriesData(basketProductsComparisonOld || []);
        return data.sort((a, b) => a.name.localeCompare(b.name));
    }, [basketProductsComparisonOld]);

    const chartEnergyMacroCategoriesData = useMemo(() => {
        return getChartEnergyMacroCategoriesData(basketProductsComparisonNew || [], selectedMacro);
    }, [basketProductsComparisonNew, selectedMacro]);

    const chartComparisonEnergyMacroCategoriesData = useMemo(() => {
        return getChartEnergyMacroCategoriesData(basketProductsComparisonOld || [], selectedMacro);
    }, [basketProductsComparisonOld, selectedMacro]);

    const chartEnergyMicroCategoriesData = useMemo(() => {
        return getChartEnergyMicroCategoriesData(basketProductsComparisonNew || [], selectedMicro);
    }, [basketProductsComparisonNew, selectedMicro]);

    const chartComparisonEnergyMicroCategoriesData = useMemo(() => {
        return getChartEnergyMicroCategoriesData(basketProductsComparisonOld || [], selectedMicro);
    ...
    useEffect(() => {
        if (currentTab === 'energy' && selectedCategories.major.length === 0) {
            if (chartEnergyCategoriesData.length > 0) {
                ...
                const defaultCategory = chartEnergyCategoriesData.reduce((max, category) =>
                    category.value > max.value ? category : max,
                ).name;

                setSelectedCategories({ major: [defaultCategory], sub: [] });
            }
        } else if (currentTab === 'macro' && selectedCategories.major.length === 0) {
            if (chartEnergyMacroCategoriesData.length > 0) {
                // Find the category with the highest value
                const defaultCategory = chartEnergyMacroCategoriesData.reduce((max, category) =>
                    category.value > max.value ? category : max,
                ).name;
                setSelectedCategories({ major: [defaultCategory], sub: [] });
            }
        } else if (currentTab === 'micro' && selectedCategories.major.length === 0) {
            if (chartEnergyMicroCategoriesData.length > 0) {
                // Find the category with the highest value
                const defaultCategory = chartEnergyMicroCategoriesData.reduce((max, category) =>
                    category.value > max.value ? category : max,
                ).name;
                setSelectedCategories({ major: [defaultCategory], sub: [] });
            }
    ...
    useEffect(() => {
        if (currentTab === 'energy' && selectedSortCriteria) {
            ...
            if (!chartEnergyMacroData.find((item) => item.name === selectedSortCriteria)) {
                setSelectedSortCriteria('Kohlenhydrate');
            }
            // Calculate the percentage difference primary
            const comparisonItem = chartComparisonEnergyMacroData.find((item) => item.name === selectedSortCriteria);

            if (comparisonItem) {
                const selectedItem = chartEnergyMacroData.find((item) => item.name === selectedSortCriteria);

                if (selectedItem) {
                    const difference = ((comparisonItem.value - selectedItem.value) / selectedItem.value) * 100;
                ...
                <div className="-mx-8 flex-1 max-h-[calc(100vh-320px)] overflow-y-auto pb-6 px-8">
                    <div className="flex mt-6 px-4 justify-center w-full">
                        <div className="text-center">
                            ...
                            <h3 className="mt-2 text-sm font-semibold text-gray-900">Keine zweite Sitzung</h3>
    ...
    const range1ProductCount =
        basketProductsComparisonNew?.reduce((sum, basket) => sum + basket.products.length, 0) || 0;

    const range2ProductCount =
        basketProductsComparisonOld?.reduce((sum, basket) => sum + basket.products.length, 0) || 0;
    ...
    return (
        <div className="bg-white -ml-8 -mr-8 border-x border-gray-300 pt-6 bg-gray-50 flex flex-col flex-1 px-8 border-b h-[calc(100vh-184px)]">
            ...
            <div className="-mx-8 flex-1 max-h-[calc(100vh-300px)] overflow-y-auto pb-6 px-8">
                ...
                />
                <div>
                    {(isLoadingBasketProductsComparisonNew || isLoadingBasketProductsComparisonOld || isProcessing) && (
                        ...
                        <>
                            {currentTab === 'energy' && (
                                <>
                                    ...
                                    <br />
                                    <h4 className="text-lg font-medium mb-2">Energieverteilung aus Makronährstoffen</h4>

                                    <div className="flex items-center justify-between">
                                        <ChartEnergyMacro data={chartEnergyMacroData} rounded={false} />
                                        <DiffDot percentageDifference={percentageDifferencePrimary} variant="sort" />
                                        <ChartEnergyMacro data={chartComparisonEnergyMacroData} rounded={false} />
                                    </div>
                                </>
                            )}
                            {currentTab === 'macro' && (
                            ...
                            {currentTab === 'micro' && (
                                <>
                                    <br />
                                    <h4 className="text-lg font-medium mb-2">Weitere Nährstoffe</h4>
                                    <div className="flex items-center justify-between">
                            ...
                            <div className="flex mt-12 px-4 justify-center w-full">
                                <div className="text-center">
                                    ...
                                    <p className="mt-2 text-sm text-gray-500">
                                        Wählen Sie mindestens zwei Einkäufe aus, um Analysen anzuzeigen.
                                    </p>
                                    <p className="mt-2 text-sm text-gray-500">
