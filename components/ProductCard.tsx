...
const ProductCard: React.FC<ProductCardProps> = ({ product, variant = 'default', onRemove }) => {
    const {
    ...
    const isProductSelected = (gtin: number) => selectedBasketProductIds.some((item) => item.gtin === gtin);

    const handleProductCheckboxChange = () => {
        if (isProductSelected(product.gtin)) {
            const newSelectedBasketProductIds = selectedBasketProductIds.filter((item) => item.gtin !== product.gtin);
            setSelectedBasketProductIds(newSelectedBasketProductIds);

            const newSelectedBasketProductsFlat = selectedBasketProductsFlat.filter((p) => p.gtin !== product.gtin);
            ...
            const newSelectedBasketProductIds = [...selectedBasketProductIds, selectedBasketProductId];
            setSelectedBasketProductIds(newSelectedBasketProductIds);

            const newSelectedBasketProductsFlat = [...selectedBasketProductsFlat, product];
            setSelectedBasketProductsFlat(newSelectedBasketProductsFlat);
        }
    };
    ...
    const nutrientValue = nutrientKey ? product.nutrients[nutrientKey] : undefined;
    const roundedNutrientValue = typeof nutrientValue === 'number' ? nutrientValue.toFixed(0) : nutrientValue || 'N/A';

    const nutriScoreClass = nutriScoreColorMap[product.nutrients.nutriScore] || 'bg-gray-200 text-gray-700';
