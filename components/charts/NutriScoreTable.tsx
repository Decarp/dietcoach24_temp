...
import type { CellClassParams, ColDef, GetDataPath, ICellRendererParams } from '@ag-grid-community/core';
...
ModuleRegistry.registerModules([ClientSideRowModelModule, RowGroupingModule, RichSelectModule]);
...
export const NutriScoreTable = () => {
    ...
    const { selectedCategories, selectedBasketIds, updateCategories } = useCounterStore((state) => state);

    const { isLoading, error, data } = useQuery<NutrientTableResponseItem[]>({
        queryKey: ['nutrientTable', patientId, selectedBasketIds],
        queryFn: () => fetchNutrientTable(patientId, selectedBasketIds, session?.accessToken || ''),
    ...
    return (
        <div className="w-full mx-auto border border-gray-300">
            <div
                ...
                style={{
                    height: 'calc(60vh - var(--layout-grid-header-height) - var(--layout-grid-margin))',
                }}
            >
                <AgGridReact
                    ref={gridRef}
...
const CategoryCellRenderer: FunctionComponent<CustomCellRendererProps> = ({ value, data: { Category } }) => {
    ...
    return (
        <div className="flex flex-row-reverse items-center justify-start gap-2">
            <div className="flex flex-col">
                <span
                    className={`${
                        isMajorCategory ? 'text-sm text-gray-700 font-semibold' : 'text-sm text-gray-500'
