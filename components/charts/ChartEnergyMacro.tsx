...
export default function ChartEnergyMacro({
    ...
    const { selectedSortCriteria, setSelectedSortCriteria } = useCounterStore((state) => state);

    useEffect(() => {
        const updatedIndex = data.findIndex((item) => item.name === selectedSortCriteria);
    ...
    return (
        <div className={`rounded-lg p-4 border border-gray-300 w-full ${className}`} style={{ height: '350px' }}>
                ...
                <PieChart>
                    ...
                    >
                        {data.map((entry, index) => (
                            <Cell
                                key={`cell-${index}`}
                                fill={categoryColors[entry.name as keyof typeof categoryColors] || '#CCCCCC'}
