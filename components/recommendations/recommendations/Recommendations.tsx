...
import { DatabaseProduct, EnrichedRecommendation, Session } from '@/types/types';
...
const Recommendations = () => {
    ...
    const { data: consultationSession, refetch } = useQuery<Session>({
        ...
        queryFn: () => fetchSession(selectedSessionId ?? 0, session?.accessToken || ''),
        enabled: selectedSessionId !== null && !!session?.accessToken,
    });

    // Get all product IDs from recommendations
    const alternativeProductIds =
        consultationSession?.recommendations.flatMap((recommendation) => recommendation.suggestions.alternatives) || [];

    const currentProductIds =
        consultationSession?.recommendations.flatMap((recommendation) => recommendation.suggestions.current) || [];

    // Concatenate all product IDs and remove duplicates
    const allProductIds = Array.from(new Set([...alternativeProductIds, ...currentProductIds]));

    // Fetch all products for the given GTINs
    const { data: fullProducts } = useQuery<DatabaseProduct[]>({
        queryKey: ['products', allProductIds],
        queryFn: () => Promise.all(allProductIds.map((gtin) => fetchProduct(gtin.toString()))),
    ...
    const enrichedRecommendations: EnrichedRecommendation[] =
        consultationSession?.recommendations.map((recommendation) => {
            const enrichedCurrentProducts = recommendation.suggestions.current
                .map((gtin) => {
                    return fullProducts?.find((product) => product.gtins.includes(Number(gtin)));
                })
                .filter((product): product is DatabaseProduct => product !== undefined); // Filter out undefined

            const enrichedAlternativeProducts = recommendation.suggestions.alternatives
                .map((gtin) => {
                    return fullProducts?.find((product) => product.gtins.includes(Number(gtin)));
                })
                .filter((product): product is DatabaseProduct => product !== undefined); // Filter out undefined
    ...
    const deleteRecommendationMutation = useMutation({
        mutationFn: (recommendationId: number) => deleteRecommendation(recommendationId, session?.accessToken || ''),
        onSuccess: () => {
                ...
                <div className="shadow-inner -mx-6">
                    <div className="flex-1 mt-12 max-h-[calc(100vh-314px)] overflow-y-auto pb-6 px-6">
                        <div className="text-center">
                            ...
                            <h3 className="mt-6 text-sm font-semibold text-gray-900">Keine Sitzung ausgewählt</h3>
            ...
            <div className="pt-6 bg-gray-50 flex flex-col flex-1 px-4 sm:px-6 lg:pl-8 xl:pl-6 border-b border-gray-300">
                ...
                <div className="shadow-inner -mx-6">
                    <div className="flex-1 mt-12 max-h-[calc(100vh-314px)] overflow-y-auto pb-6 px-6">
                        <div className="text-center">
                            <ShoppingCartIcon className="mx-auto h-12 w-12 text-gray-400" />
                            <h3 className="mt-6 text-sm font-semibold text-gray-900">Keine Empfehlungen vorhanden</h3>
                            ...
                            >
                                <MagnifyingGlassIcon aria-hidden="true" className="-ml-0.5 mr-2 h-5 w-5 text-white" />
    ...
    return (
        <div className="pt-6 bg-gray-50 flex flex-col flex-1 px-4 sm:px-6 lg:pl-8 xl:pl-6 border-b border-gray-300">
            ...
            {isModalOpen && (
                <DeleteModal isOpen={isModalOpen} closeModal={closeModal} onConfirm={handleDeleteRecommendation} />
            )}
        </div>
    );
};
