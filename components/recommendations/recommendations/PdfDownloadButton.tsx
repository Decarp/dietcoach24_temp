...
import { EnrichedRecommendation, Patient, Session, DatabaseProduct } from '@/types/types';
...
const PdfDownloadButton: React.FC<PdfDownloadButtonProps> = ({ sessionData, enrichedRecommendations, patient }) => {
    const [loading, setLoading] = useState(false);

    const handleDownloadClick = async () => {
        setLoading(true);

        const enrichedRecommendationsWithBase64: EnrichedRecommendation[] = await Promise.all(
            enrichedRecommendations.map(async (rec) => {
                const currentProductsWithBase64 = await Promise.all(
                    rec.suggestions.current.map(async (product) => {
                        const base64Image = (await fetchBase64Image(product.imageUrl)) || '';
