...
const SessionsComp = () => {
    ...
    const { selectedSessionId, selectedSessionIndex, setSelectedSessionId, setSelectedSessionIndex } = useCounterStore(
        (state) => state,
    );
    ...
    useEffect(() => {
        if (selectedSessionId !== null && !sessionTimestamps.some((s) => s.sessionId === selectedSessionId)) {
    ...
    }, [sessionTimestamps, selectedSessionId, setSelectedSessionId, setSelectedSessionIndex]);
    ...
    const deleteMutation = useMutation({
        mutationFn: (sessionId: number) => deleteSession(sessionId, session?.accessToken || ''),
        onSuccess: () => {
    ...
    const handleSessionClick = (sessionId: number) => {
        ...
        setSelectedSessionIndex(sessionTimestamps.findIndex((s) => s.sessionId === sessionId));
    };
        ...
        <div className="pt-6 -ml-8 bg-white flex flex-col border-b border-x border-gray-300 w-[300px] h-[calc(100vh-185px)]">
            ...
            <div className="bg-white flex-1 overflow-y-auto min-h-0 shadow-inner">
                ...
                <ul aria-label="Sessions List" className="overflow-y-auto">
                    {[...sessionTimestamps].reverse().map(({ sessionId, timestamp }, index) => (
                        <li
                            key={sessionId}
                                ...
                                <div className="min-w-0 flex-1">
                                    <p
                                        className={classNames(
                                            ...
                                            selectedSessionId === sessionId ? 'text-white' : 'text-gray-900',
                                        )}
                                    >
                                    ...
                                    <p
                                        className={classNames(
                                            ...
                                            selectedSessionId === sessionId ? 'text-white' : 'text-gray-500',
                                        )}
                                    >
            ...
            {isModalOpen && (
                <DeleteModal isOpen={isModalOpen} closeModal={closeModal} onConfirm={handleDeleteSession} />
            )}
        </div>
    );
};
