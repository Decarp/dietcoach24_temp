...
const sortProducts = (products: BasketProductFlat[], selectedSortCriteria: string, ascending: boolean) => {
...
const groupProductsByGtin = (products: BasketProductFlat[]): BasketProductFlat[] => {
...
const Products = () => {
    ...
    const newBasketProductsFlat: BasketProductFlat[] = basketProducts.flatMap((basket) => {
    ...
    useEffect(() => {
        if (JSON.stringify(newBasketProductsFlat) !== JSON.stringify(basketProductsFlat)) {
    ...
    const sortedProducts = sortProducts(groupedProducts, selectedSortCriteria, ascending);
    ...
    const handleSelectAllChange = () => {
        if (areAllProductsSelected) {
            ...
            sortedProducts.forEach((product) => {
                if (!selectedBasketProductIds.some((item) => item.gtin === product.gtin)) {
    ...
    return (
        <div
            className={`relative pt-6 -mr-8 bg-white border-r flex flex-col shrink-0 border-t border-b border-gray-300 min-w-60 md:max-w-96 xl:w-96 lg:border-t-0 lg:pr-8 xl:pr-6 h-[calc(100vh-185px)] transform transition-all duration-1000 ease-in-out ${
                hideProducts ? 'opacity-0 translate-x-10 -mr-96' : 'opacity-100 translate-x-0'
            }`}
        >
            ...
            <div className="border-b border-gray-300 px-6 mt-[1px] -mr-6 pb-2 flex gap-x-8 items-center">
                <FilterPopover categoriesWithSub={categoriesWithSub} selectedCategories={selectedCategories} />
            ...
            <div className="-mr-6 flex-1 overflow-y-auto min-h-0 shadow-inner border-b border-gray-300">
                ...
                {selectedCategories.major.length === 0 && !isLoading && selectedCategories.sub.length === 0 && (
                    <div className="flex mt-12 px-4">
                        ...
                        <div className="text-center">
                            ...
                            <p className="mt-2 text-sm text-gray-500">
                                Wählen Sie mindestens eine Lebensmittelkategorie aus, um die Lebensmittel anzuzeigen.
                            </p>
                        </div>
                    </div>
                )}
            ...
            {(selectedCategories.major.length !== 0 || selectedCategories.sub.length !== 0) && (
                <div className="flex justify-end py-6">
                    <Button onClick={() => handleDrawerOpen(true)} icon={<PlusIcon className="h-5 w-5" />}>
            ...
            <CompletedPopup open={completedPopupOpen} setOpen={setCompletedPopupOpen} />
        </div>
    );
};
