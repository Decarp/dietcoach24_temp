...
const Baskets = () => {
    ...
    const { startTimestamp, endTimestamp } = getBasketTimestamps(patient, sessions);

    const { isLoading, error, data } = useQuery<Basket[]>({
        queryKey: ['baskets', patientId, startTimestamp, endTimestamp],
        queryFn: () => fetchBaskets(patientId, startTimestamp, endTimestamp || '', session?.accessToken || ''),
    ...
    const selectLastNWeeks = useCallback(() => {
        if (Object.keys(baskets).length > 0) {
            ...
            const selectedBaskets = Object.values(baskets)
                .flat()
                .filter((basket: Basket) => isAfter(fromUnixTime(basket.timestamp), earliestDate))
    ...
    const handleBasketCheckboxChange = (basketId: string) => {
        if (selectedBasketIds.includes(basketId)) {
            setSelectedBasketIds(selectedBasketIds.filter((item) => item !== basketId));
            setSelectedBasketProductIds(selectedBasketProductIds.filter((product) => product.basketId !== basketId));
        } else {
    ...
    const handleSelectAllChange = (letter: string, selectAll: boolean) => {
        ...
        if (selectAll) {
            ...
            setSelectedBasketIds(selectedBasketIds.filter((id) => !basketIds.includes(id)));
        }
    };
    ...
    return (
        <div
            className={`pt-6 -ml-8 bg-white border-l flex flex-col border-b border-gray-300 min-w-64 xl:w-64 xl:shrink-0 h-[calc(100vh-185px)] transform transition-all duration-1000 ease-in-out ${
                hideBaskets ? 'opacity-0 -translate-x-10 -ml-[256px]' : 'opacity-100 translate-x-0'
            }`}
        >
            <BasketsHeader
                baskets={baskets}
                                    ...
                                    <div className="pl-8 sticky top-0 z-10 border-y border-b-gray-200 border-t-gray-100 bg-gray-50 px-3 py-1.5 text-sm font-semibold leading-6 text-gray-900">
                                        <div className="flex justify-between items-center">
                                            ...
                                            <input
                                                ...
                                                checked={allSelected}
                                                onChange={(e) => handleSelectAllChange(letter, e.target.checked)}
                                            />
                                        </div>
                                    </div>
                                        ...
                                        {baskets[letter].map((basket) => (
                                            ...
                                            >
                                                ...
                                                <input
                                                    ...
                                                    onChange={() => handleBasketCheckboxChange(basket.basketId)}
                                                />
                                            </li>
                                        ))}
                                    </ul>
