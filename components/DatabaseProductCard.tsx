...
const nutrientKeyMap: { [key: string]: keyof DatabaseNutrients } = {
    ...
    Proteine: 'proteinsG',
    Fette: 'fatsG',
    'Gesättigte Fettsäuren': 'saturatedFatsG',
    Kohlenhydrate: 'carbohydratesG',
    Zucker: 'sugarsG',
    Nahrungsfasern: 'fibersG',
    Salz: 'saltG',
};
...
const DatabaseProductCard: React.FC<DatabaseProductCardProps> = ({
    ...
    const nutriScoreClass =
        nutriScoreColorMap[product.nutriScoreV2023Detail.nutriScoreCalculated] || 'bg-gray-200 text-gray-700';

    const isSelected = selectedAlternativeProducts.some((altProduct) => altProduct.productId === product.productId);
                        ...
                        {Object.entries(nutrientKeyMap).map(([label, key]) => (
                            <div
                                ...
                                className={classNames(
                                    ...
                                    isSelected && variant === 'default' ? 'text-white' : 'text-gray-700',
                                    selectedNutrient === label && selectedNutrientColor
                                )}
                            >
                                <p>{label === 'Gesättigte Fettsäuren' ? 'Ges. Fette' : label}: </p>
