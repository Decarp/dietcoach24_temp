...
export type CounterActions = {
    ...
    updateCategories: (category: string, level: 'major' | 'sub', replace?: boolean) => void;
...
export const createCounterStore = (initState: CounterState = defaultInitState) => {
    return createStore<CounterStore>()((set) => ({
        ...
        setSelectedCategories: (cats: CategorySelection) =>
            set((state) => {
                const newSelectedBasketProductIds = state.selectedBasketProductIds.filter((id) =>
                ...
                const newSelectedBasketProductsFlat = state.basketProductsFlat.filter((product) =>
                    newSelectedBasketProductIds.some(
                        (id) => product.basketId === id.basketId && product.gtin === id.gtin,
                    ),
                );
        ...
        setSelectedSortCriteria: (criteria: string) => set(() => ({ selectedSortCriteria: criteria })),
        setSelectedBasketIds: (ids: string[]) => set(() => ({ selectedBasketIds: ids })),
        setSelectedComparisonBasketIds: (ids: string[]) => set(() => ({ selectedComparisonBasketIds: ids })),
        setSelectedBasketProductIds: (ids: SelectedBasketProductId[]) =>
            set((state) => {
                const newSelectedBasketProductsFlat = state.basketProductsFlat.filter((product) =>
                    ids.some((id) => product.basketId === id.basketId && product.gtin === id.gtin),
                );
        ...
        setBasketProducts: (products: BasketProduct[]) => set(() => ({ basketProducts: products })),
        setBasketProductsFlat: (products: BasketProductFlat[]) => set(() => ({ basketProductsFlat: products })),
        ...
        setSelectedSessionId: (id: number | null) => set(() => ({ selectedSessionId: id })),
        setSelectedSessionIndex: (index: number | null) => set(() => ({ selectedSessionIndex: index })),
        updateCategories: (category: string, level: 'major' | 'sub', replace: boolean = false) =>
            set((state) => {
                ...
                if (replace) {
                    ...
                    if (level === 'major') {
                        if (newCategories.major.includes(category)) {
                            ...
                            newCategories.sub = Array.from(new Set([...newCategories.sub, ...newSubCategories]));
                        }
                    } else {
                        if (newCategories.sub.includes(category)) {
                            ...
                            if (
                                ...
                                newCategories.major = newCategories.major.filter((i) => i !== majorCategory);
                            }
                        } else {
                            // Select sub-category
        ...
        setSelectedMacro: (macro: MacroCategory) => set(() => ({ selectedMacro: macro })),
        setSelectedMicro: (micro: MicroCategory) => set(() => ({ selectedMicro: micro })),
