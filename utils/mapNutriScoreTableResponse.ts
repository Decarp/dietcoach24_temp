...
export const mapNutriScoreTableResponse = (data: NutrientTableResponseItem[]): NutrientTableItem[] => {
    const sortedByMajorCategory = data.sort((a, b) => a.MajorCategory.de.localeCompare(b.MajorCategory.de));

    const filtered = sortedByMajorCategory.filter((item) => item.MajorCategory.de !== 'Ausgeschlossen');
