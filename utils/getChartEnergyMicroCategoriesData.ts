...
const mapChartEnergyMicroCategoriesResponse = (
    ...
    const totalGrams = chartMicroCategoriesResponse.reduce((sum, item) => sum + (item.grams || 0), 0);
...
export const getChartEnergyMicroCategoriesData = (
    ...
    const dynamicChartEnergyMicroCategoriesResponse = aggregateMicroCategories(productsFlattened, selectedMicro);

    return mapChartEnergyMicroCategoriesResponse(dynamicChartEnergyMicroCategoriesResponse);
};
